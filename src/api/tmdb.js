...
export async function getJSON(path, params = {}) {
  ...
  if (!res.ok) {
    throw new Error(`TMDB ตอบกลับ ${res.status}`);
  }
  return res.json();}
...
export function toMovie(m, genreMap = {}) {
  ...
  return {
    ...
    title: m.title,
    titleTh: null,
    genre: m.genres?.[0]?.name ?? genreMap[m.genre_ids?.[0]] ?? null,
  ...
  };   // ชั่วคราว: ส่งกลับตามเดิม การ์ดจะขึ้นไม่ครบเพราะชื่อ field ไม่ตรง
}
